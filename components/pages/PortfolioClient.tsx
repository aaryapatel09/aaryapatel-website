      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
          ...
          <p className="text-lg font-mono text-gray-300 max-w-3xl">
            A collection of projects spanning software engineering, F1 systems design, and powertrain development.
          </p>
        </motion.section>
          ...
          {categories.map((category, index) => (
            <motion.button
              ...
              className={`px-6 py-2 font-racing tracking-wider text-sm border-2 transition-colors ${
                selectedCategory === category ? 'border-white text-white bg-white/10' : 'border-white/30 text-white/70 hover:border-white hover:text-white'
              }`}
            >
          ...
          <AnimatePresence mode="wait">
            {filteredProjects.map((project, index) => (
              <motion.button
                ...
                className={
                  isMobile
                    ? 'w-full appearance-none bg-transparent border-0 p-0 text-left'
                    : 'w-full cursor-grab active:cursor-grabbing appearance-none bg-transparent border-0 p-0 text-left'
                }
                onClick={(e) => {
                  ...
                  const url = project.githubUrl ?? 'https://github.com/aaryapatel09'
