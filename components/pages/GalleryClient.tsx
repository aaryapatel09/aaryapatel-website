...
export default function GalleryClient() {
  ...
  // Focus management for the gallery "lightbox" modal
  useEffect(() => {
  ...
  const handleDialogKeyDown = (e: ReactKeyboardEvent<HTMLDivElement>) => {
    ...
    const focusable = Array.from(
      ...
      // Only consider elements that are actually visible / focusable.
    ...
    <div className="pt-24 pb-12 min-h-screen bg-black text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
          <h1 className="text-5xl md:text-7xl font-racing tracking-wider text-white mb-6">GALLERY</h1>
          ...
          {filters.map((filter, index) => (
            <motion.button
              ...
              className={`px-4 py-2 font-racing text-sm border-2 transition-colors ${
                galleryFilter === filter.id ? 'border-white text-white bg-white/10' : 'border-white/30 text-white/70 hover:border-white hover:text-white'
              }`}
            >
              {filter.label}
            </motion.button>
            ...
            <AnimatePresence mode="wait">
              {sortedAndFilteredItems.map((item, index) => (
                <motion.button
                  ...
                  aria-label={`Open ${item.title} in a dialog`}
                  className="cursor-pointer break-inside-avoid mb-6 appearance-none bg-transparent border-0 p-0 text-left w-full"
                >
                  <DashboardCard className="interactive-element overflow-hidden p-0">
                    {item.image ? (
                      ...
                      >
                        <div className="text-black/30 text-sm font-mono">{item.title}</div>
                      </motion.div>
                    )}
                    <div className="p-4">
                      <motion.h3
          ...
          >
            ...
            >
              ...
              <motion.div
                ...
                transition={{ delay: 0.3 }}
              >
                <h2 id="gallery-modal-title" className="text-2xl md:text-3xl font-racing text-white mb-3">
                  {selectedItem.title}
                </h2>
